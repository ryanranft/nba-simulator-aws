...
def get_espn_game_ids_for_season(season: int) -> List[str]:
    ...
    try:
        result = subprocess.run(
            ["aws", "s3", "ls", f"s3://nba-sim-raw-data-lake/pbp/", "--recursive"],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
def process_batch(
    ...
    for i, game_id in enumerate(game_ids, 1):
        try:
            ...
            if not snapshots:
                logger.warning(
                    f"  [{i}/{len(game_ids)}] {game_id}: No snapshots generated"
                )
                stats["errors"] += 1
                continue
...
def main():
    ...
    parser.add_argument(
        "--start-season", type=int, default=1993, help="Start season (default: 1993)"
    )
    parser.add_argument(
        "--end-season", type=int, default=2025, help="End season (default: 2025)"
    )
    ...
    logger.info(
        f"RDS storage: {'Enabled' if args.save_rds else 'Disabled (JSON only)'}"
    )
    logger.info("")
    ...
    logger.info(
        f"Processing seasons: {min(seasons)} - {max(seasons)} ({len(seasons)} seasons)"
    )
    logger.info("")
    ...
    logger.info(
        f"Success rate: {overall_stats['processed']/overall_stats['total_games']*100:.1f}%"
    )
        ...
        json.dump(
            {
                ...
                "success_rate": overall_stats["processed"]
