...
for testing the plus/minus system without running full Phase 2 processors.
    ...
    for event_num in range(num_events):
        ...
        if event_num % 5 == 0:  # Score every ~5 events
            ...
            if scoring_team == "home":
                ...
                scorers = [
                    p for p in home_players[:5] if player_stats[p[0]]["on_court"]
                ]
                if scorers:
                ...
                away_score += points
                scorers = [
                    p for p in away_players[:5] if player_stats[p[0]]["on_court"]
                ]
                if scorers:
                    scorer = scorers[event_num % len(scorers)]
                    player_stats[scorer[0]]["points"] += points
        ...
        if event_num > 0 and event_num % 40 == 0:
            ...
            off_court_home = [
                p for p in home_players if not player_stats[p[0]]["on_court"]
            ]
