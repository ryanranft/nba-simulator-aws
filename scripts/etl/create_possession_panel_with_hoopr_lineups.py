...
class LineupTracker:
    ...
    def initialize_from_hoopr(self, conn):
        ...
        if not starters:
            logger.warning(
                f"No hoopR starters found for hoopr_game_id {self.hoopr_game_id} (Kaggle game_id: {self.kaggle_game_id})"
            )
            return False
        ...
        logger.debug(
            f"Game {self.kaggle_game_id}: Home starters = {len(self.home_lineup)}, Away starters = {len(self.away_lineup)}"
        )

        # Validate we have 5 starters per team
        if len(self.home_lineup) != 5 or len(self.away_lineup) != 5:
            logger.warning(
                f"Game {self.kaggle_game_id}: Incomplete starters (Home: {len(self.home_lineup)}, Away: {len(self.away_lineup)})"
            )
            return False
    ...
    def update_lineup(self, event_text: str, team_id: str):
        ...
        if "enters the game for" in text_lower:
            ...
            if len(parts) == 2:
                ...
                logger.debug(
                    f"Substitution detected: {player_in_name} in, {player_out_name} out"
                )
...
class PossessionDetector:
    ...
    def is_possession_end(
        ...
        if next_event_text and prev_score != curr_score:
            ...
            is_made_shot = (
                "made" in text_lower
                or "layup" in text_lower
                or "dunk" in text_lower
                or ("jumper" in text_lower and "miss" not in text_lower)
            )
...
class PossessionPanelBuilder:
    ...
    def get_games_to_process(self, limit: int = None) -> list:
        ...
        logger.info(
            "Fetching game list (Kaggle events + hoopR starters matched by date)..."
        )
        ...
        logger.info(
            f"✓ Found {len(rows)} games with Kaggle events + hoopR starters (query took {elapsed:.1f}s)"
        )
    ...
    def process_game(self, game: dict) -> list:
        ...
        if not lineup_tracker.initialize_from_hoopr(self.conn):
            logger.warning(
                f"Game {kaggle_game_id}: Could not initialize lineups from hoopR, skipping"
            )
            return []
        ...
        for idx, row in events_df.iterrows():
            ...
            # Extract event text
            event_text = (
                event_data.get("home_description", "")
                or event_data.get("visitor_description", "")
                or event_data.get("neutral_description", "")
                or ""
            )
            ...
            if next_idx in events_df.index:
                ...
                if isinstance(next_event_data, dict):
                    next_event_text = (
                        next_event_data.get("home_description", "")
                        or next_event_data.get("visitor_description", "")
                        or next_event_data.get("neutral_description", "")
                        or ""
                    )
            ...
            if is_possession_end and len(current_possession["events"]) > 0:
                # Get current lineups
                offensive_team_id = (
                    current_possession["team_id"] or game["home_team_id"]
                )
        ...
        logger.debug(
            f"Game {kaggle_game_id}: extracted {len(possessions)} possessions with lineups"
        )
        return possessions
...
def main():
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            ...
            # Lineup coverage
            lineup_cols = [f"offense_player_{i}" for i in range(1, 6)] + [
                f"defense_player_{i}" for i in range(1, 6)
            ]
