...
def extract_game_info(data, game_id) -> Optional[Dict]:
    ...
    try:
        ...
        return {
            ...
            'home_team_id': str(home_team['id']) if home_team and 'id' in home_team else None,
            'home_team_abbrev': home_team.get('abbrev', '') if home_team else '',
            'home_stats': home_stats or {},
            'away_team_id': str(away_team['id']) if away_team and 'id' in away_team else None,
                ...
                for player in stat_group.get('athlts', []):
                    try:
                        ...
                        players_data.append((
                            ...
                            str(stats_dict.get('fieldGoalsMade-fieldGoalsAttempted', '0-0')),
                            str(stats_dict.get('threePointFieldGoalsMade-threePointFieldGoalsAttempted', '0-0')),
                            str(stats_dict.get('freeThrowsMade-freeThrowsAttempted', '0-0')),
                        ...
                        print(f"  Warning: Error processing player in game {game_id}: {e}")
                        continue
...
def process_year(year: int, dry_run: bool = False) -> Dict:
    ...
    if not dry_run:
        try:
            ...
            return {'processed': 0, 'inserted_teams': 0, 'inserted_players': 0, 'skipped': 0, 'errors': 0}
    ...
    # Process each game
    stats = {'processed': 0, 'inserted_teams': 0, 'inserted_players': 0, 'skipped': 0, 'errors': 0}
    ...
    for i, game_id in enumerate(game_ids, 1):
        ...
        try:
            ...
            if game_info and not dry_run:
                ...
                """, (
                    game_info['game_id'], game_info['game_date'],
                    game_info['home_team_id'], game_info['home_team_abbrev'],
                    home_stats.get('score', 0), home_stats.get('field_goals', '0-0'), home_stats.get('field_goals_pct', 0),
                    home_stats.get('three_pointers', '0-0'), home_stats.get('three_pointers_pct', 0),
                    home_stats.get('free_throws', '0-0'), home_stats.get('free_throws_pct', 0),
                    home_stats.get('offensive_rebounds', 0), home_stats.get('defensive_rebounds', 0),
                    home_stats.get('total_rebounds', 0), home_stats.get('assists', 0), home_stats.get('steals', 0),
                    home_stats.get('blocks', 0), home_stats.get('turnovers', 0), home_stats.get('fouls', 0),
                    game_info['away_team_id'], game_info['away_team_abbrev'],
                    away_stats.get('score', 0), away_stats.get('field_goals', '0-0'), away_stats.get('field_goals_pct', 0),
                    away_stats.get('three_pointers', '0-0'), away_stats.get('three_pointers_pct', 0),
                    away_stats.get('free_throws', '0-0'), away_stats.get('free_throws_pct', 0),
                    away_stats.get('offensive_rebounds', 0), away_stats.get('defensive_rebounds', 0),
                    away_stats.get('total_rebounds', 0), away_stats.get('assists', 0), away_stats.get('steals', 0),
                    away_stats.get('blocks', 0), away_stats.get('turnovers', 0), away_stats.get('fouls', 0),
                    game_info['attendance'], game_info['game_state'],
                    game_info['referee1_name'], game_info['referee1_position'],
                    game_info['referee2_name'], game_info['referee2_position'],
                    game_info['referee3_name'], game_info['referee3_position']
                ))
            ...
            if i % 100 == 0:
                print(f"  Progress: {i}/{len(game_ids)} games | {stats['inserted_players']} players extracted")
...
def main():
    parser = argparse.ArgumentParser(description='Extract NBA box score data from S3 to RDS PostgreSQL')
    ...
    # Process each year
    total_stats = {'processed': 0, 'inserted_teams': 0, 'inserted_players': 0, 'skipped': 0, 'errors': 0}
