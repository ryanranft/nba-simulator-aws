...
AGENT_SCRIPT="$SCRIPT_DIR/phase_1_11_deduplication_agent.py"
LOG_DIR="$PROJECT_ROOT/logs"
PID_FILE="$PROJECT_ROOT/logs/phase_1_11_deduplication_agent.pid"
...
check_prerequisites() {
    ...
    python3 -c "import asyncio, logging, json, time, pandas, psycopg2, pathlib, datetime, typing, sys, os, collections, hashlib" 2>/dev/null || {
    ...
    # Check for data source directories
    data_sources_found=0
    if [ -d "/tmp/hoopr_phase1" ]; then
        log_info "Found hoopR data directory"
        data_sources_found=$((data_sources_found + 1))
    fi
    if [ -d "/tmp/phase_1_7_nba_stats" ]; then
        log_info "Found NBA API data directory"
        data_sources_found=$((data_sources_found + 1))
    fi
    if [ -d "/tmp/bbref_tier_1" ]; then
        log_info "Found Basketball Reference data directory"
        data_sources_found=$((data_sources_found + 1))
    fi

    if [ $data_sources_found -eq 0 ]; then
        log_warning "No data source directories found - agent may not have data to process"
    else
        log_info "Found $data_sources_found data source directories"
    fi
...
start_agent() {
    ...
    cd "$PROJECT_ROOT"
    nohup python3 "$AGENT_SCRIPT" > "$LOG_DIR/phase_1_11_deduplication_agent.out" 2>&1 &
    PID=$!
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        log_info "Check logs for details: tail -f $LOG_DIR/phase_1_11_deduplication_agent.out"
        exit 1
    fi
}
...
status_agent() {
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        if [ -f "$LOG_DIR/phase_1_11_deduplication_agent.log" ]; then
            log_info "Recent log entries:"
            tail -5 "$LOG_DIR/phase_1_11_deduplication_agent.log" | sed 's/^/  /'
        fi
        ...
        if [ -f "$OUTPUT_DIR/deduplication_report.json" ]; then
            log_info "Deduplication report summary:"
            python3 -c "
import json
try:
    with open('$OUTPUT_DIR/deduplication_report.json', 'r') as f:
        report = json.load(f)
    ...
    print(f'  Records deduplicated: {summary.get(\"records_deduplicated\", 0)}')
    print(f'  Total errors: {summary.get(\"total_errors\", 0)}')
    print(f'  Duration: {summary.get(\"deduplication_duration\", \"Unknown\")}')
...
monitor_agent() {
    ...
    # Monitor log file
    tail -f "$LOG_DIR/phase_1_11_deduplication_agent.log"
}
...
show_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  start    - Start the multi-source deduplication agent"
    echo "  stop     - Stop the agent"
    echo "  status   - Show agent status and deduplication results"
    ...
    echo ""
    echo "Multi-Source Deduplication includes:"
    echo "  - ESPN data (primary source)"
    echo "  - hoopR data (secondary source)"
    echo "  - NBA API data (official source)"
    echo "  - Basketball Reference data (historical source)"
    echo ""
    echo "Process:"
    echo "  1. Load data from all sources"
    echo "  2. Identify duplicate records"
    echo "  3. Resolve conflicts using confidence scoring"
    echo "  4. Generate deduplication report"
}
