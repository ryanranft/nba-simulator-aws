...
AGENT_SCRIPT="$SCRIPT_DIR/phase_1_0_quality_agent.py"
LOG_DIR="$PROJECT_ROOT/logs"
PID_FILE="$PROJECT_ROOT/logs/phase_1_0_quality_agent.pid"
...
check_prerequisites() {
    ...
    python3 -c "import asyncio, aiohttp, ssl, logging, json, time, boto3, pandas, pathlib, datetime, typing, sys, os, collections" 2>/dev/null || {
        log_error "Required Python packages are not installed"
        log_info "Please install: pip install aiohttp boto3 pandas"
        exit 1
    }
    ...
    if ! aws sts get-caller-identity &> /dev/null; then
        log_warning "AWS credentials not configured - S3 analysis may fail"
    fi
...
start_agent() {
    ...
    cd "$PROJECT_ROOT"
    nohup python3 "$AGENT_SCRIPT" > "$LOG_DIR/phase_1_0_quality_agent.out" 2>&1 &
    PID=$!
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        log_info "Check logs for details: tail -f $LOG_DIR/phase_1_0_quality_agent.out"
        exit 1
    fi
}
...
status_agent() {
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        if [ -f "$LOG_DIR/phase_1_0_quality_agent.log" ]; then
            log_info "Recent log entries:"
            tail -5 "$LOG_DIR/phase_1_0_quality_agent.log" | sed 's/^/  /'
        fi
        ...
        if [ -f "$OUTPUT_DIR/data_quality_report.json" ]; then
            log_info "Quality report summary:"
            python3 -c "
import json
try:
    with open('$OUTPUT_DIR/data_quality_report.json', 'r') as f:
        report = json.load(f)
    ...
    print(f'  Duration: {summary.get(\"analysis_duration\", \"Unknown\")}')
...
monitor_agent() {
    ...
    # Monitor log file
    tail -f "$LOG_DIR/phase_1_0_quality_agent.log"
}
...
show_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  start    - Start the data quality analysis agent"
    echo "  stop     - Stop the agent"
    echo "  status   - Show agent status and analysis results"
    ...
    echo "  - S3 data coverage analysis"
    echo "  - Local data file analysis"
    echo "  - Data gap identification"
