...
def get_all_unique_games(espn_conn, hoopr_conn, espn_to_hoopr, year: Optional[int] = None) -> List[Dict]:
    ...
    """
    if year:
        query = query.replace("GROUP BY", f"WHERE strftime('%Y', s.game_date) = '{year}' GROUP BY")
    ...
    espn_only = sum(1 for g in unique_games.values() if g['has_espn'] and not g['has_hoopr'])
    hoopr_only = sum(1 for g in unique_games.values() if not g['has_espn'] and g['has_hoopr'])
...
def print_summary(unified_conn):
    ...
    cursor.execute("SELECT COUNT(*) FROM source_coverage WHERE has_espn = 1 AND has_hoopr = 1;")
    both_sources = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM source_coverage WHERE has_espn = 1 AND has_hoopr = 0;")
    espn_only = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM source_coverage WHERE has_espn = 0 AND has_hoopr = 1;")
...
Result:
  ...
  - Gap handling (use available source)
    ...
    parser.add_argument(
        '--year',
        type=int,
        help='Build for specific year only'
    )
