...
# Database connection
DB_HOST = os.environ.get('DB_HOST', 'nba-sim-db.ck96ciigs7fy.us-east-1.rds.amazonaws.com')
...
def main():
    ...
    for col_name, col_type in missing_columns:
        try:
            cursor.execute(f"ALTER TABLE games ADD COLUMN IF NOT EXISTS {col_name} {col_type}")
            conn.commit()
            added += 1
