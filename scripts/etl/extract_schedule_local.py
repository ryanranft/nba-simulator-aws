    ...
    try:
        ...
        for game in all_games:
            ...
            if broadcasts:
                ...
                national = next((b for b in broadcasts if b.get('market') == 'National'), None)
                home_broadcast = next((b for b in broadcasts if b.get('market') == 'Home'), None)
            ...
            game_record = {
                ...
                'periods': game.get('format', {}).get('regulation', {}).get('periods', 4),
...
def process_year(year: int, dry_run: bool = False) -> Dict:
    ...
    for i, s3_key in enumerate(s3_files, 1):
        try:
            ...
            if i % 100 == 0:
                print(f"  Progress: {i}/{len(s3_files)} files | {len(games_to_insert)} games extracted")
    ...
    if len(games_to_insert) != len(unique_games):
        print(f"⚠️  Removed {len(games_to_insert) - len(unique_games)} duplicate game_ids")
    ...
    if not dry_run and games_to_insert:
        try:
            ...
            values = [
                (
                    game['game_id'], game['game_date'], game['game_time'], game['season'],
                    game['home_team_id'], game['home_team_abbrev'], game['home_team_name'], game['home_team_short_name'],
                    game['home_team_logo'], game['home_team_color'], game['home_team_alt_color'], game['home_team_uid'],
                    game['home_team_record_summary'], game['home_team_standing_summary'], game['home_team_location'],
                    game['home_score'], game['home_team_is_winner'], game['home_team_leader_name'], game['home_team_leader_stat'],
                    game['away_team_id'], game['away_team_abbrev'], game['away_team_name'], game['away_team_short_name'],
                    game['away_team_logo'], game['away_team_color'], game['away_team_alt_color'], game['away_team_uid'],
                    game['away_team_record_summary'], game['away_team_standing_summary'], game['away_team_location'],
                    game['away_score'], game['away_team_is_winner'], game['away_team_leader_name'], game['away_team_leader_stat'],
                    game['venue'], game['venue_id'], game['venue_city'], game['venue_state'], game['venue_country'], game['venue_indoor'],
                    game['game_status'], game['status_id'], game['status_state'], game['status_detail'],
                    game['broadcast_name'], game['broadcast_market'], game['broadcast_type'], game['broadcast_count'],
                    game['completed'], game['is_tie'], game['periods'], game['time_valid'], game['game_link'], game['header_postfix'], game['has_tickets']
                )
                for game in games_to_insert
            ]
            ...
            print(f"✅ Inserted {stats['inserted']} games with full metadata into database")
...
def main():
    parser = argparse.ArgumentParser(description='Extract NBA schedule data from S3 to RDS PostgreSQL')
    parser.add_argument('--year', type=int, help='Process single year (e.g., 1993)')
    parser.add_argument('--year-range', type=str, help='Process year range (e.g., 1993-2025)')
