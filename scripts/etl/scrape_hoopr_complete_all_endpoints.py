...
class ComprehensiveNBAScraper:
    ...
    def scrape_bulk_loaders(self, seasons):
        ...
        player_box_data = self.call_endpoint_safe(load_nba_player_boxscore, seasons=seasons)
        if player_box_data is not None and len(player_box_data) > 0:
            season_str = "_".join(map(str, seasons))
            player_box_file = self.output_dir / 'bulk_player_box' / f'player_box_seasons_{season_str}.json'
        ...
        if team_box_data is not None and len(team_box_data) > 0:
            season_str = "_".join(map(str, seasons))
            team_box_file = self.output_dir / 'bulk_team_box' / f'team_box_seasons_{season_str}.json'
        ...
        if schedule_data is not None and len(schedule_data) > 0:
            season_str = "_".join(map(str, seasons))
            schedule_file = self.output_dir / 'bulk_schedule' / f'schedule_seasons_{season_str}.json'
    ...
    def scrape_game_endpoints(self, game_id, season):
        ...
        endpoints_to_call = [
            ('boxscore_traditional', nba.nba_boxscoretraditionalv2, {'game_id': game_id}),
            ('boxscore_traditional', nba.nba_boxscoretraditionalv3, {'game_id': game_id}),
            ...
            ('boxscore_fourfactors', nba.nba_boxscorefourfactorsv2, {'game_id': game_id}),
            ('boxscore_fourfactors', nba.nba_boxscorefourfactorsv3, {'game_id': game_id}),
    ...
    def scrape_league_endpoints(self, season):
        ...
        league_endpoints = [
            ...
            ('league_dash_player_stats', nba.nba_leaguedashplayerstats,
             {'season': season_str, 'per_mode_detailed': 'PerGame'}),
            ('league_dash_player_clutch', nba.nba_leaguedashplayerclutch,
             {'season': season_str}),
            ('league_dash_player_bio', nba.nba_leaguedashplayerbiostats,
             {'season': season_str}),
            ('league_dash_player_shot_locations', nba.nba_leaguedashplayershotlocations,
             {'season': season_str}),

            # Team stats dashboards
            ('league_dash_team_stats', nba.nba_leaguedashteamstats,
             {'season': season_str, 'per_mode_detailed': 'PerGame'}),
            ('league_dash_team_clutch', nba.nba_leaguedashteamclutch,
             {'season': season_str}),
            ('league_dash_team_shot_locations', nba.nba_leaguedashteamshotlocations,
             {'season': season_str}),

            # Lineups
            ('league_dash_lineups', nba.nba_leaguedashlineups,
             {'season': season_str, 'group_quantity': 5}),

            # Player tracking
            ('league_dash_pt_stats', nba.nba_leaguedashptstats,
             {'season': season_str, 'pt_measure_type': 'SpeedDistance'}),
            ('league_dash_pt_defend', nba.nba_leaguedashptdefend,
             {'season': season_str, 'defense_category': 'Overall'}),
            ('league_dash_pt_team_defend', nba.nba_leaguedashptteamdefend,
             {'season': season_str, 'defense_category': 'Overall'}),
            ('league_dash_opponent_shots', nba.nba_leaguedashoppptshot,
             {'season': season_str}),

            # Hustle stats
            ('league_hustle_player', nba.nba_leaguehustlestatsplayer,
             {'season': season_str}),
            ('league_hustle_team', nba.nba_leaguehustlestatsteam,
             {'season': season_str}),
            ('league_hustle_player_leaders', nba.nba_leaguehustlestatsplayerleaders,
             {'season': season_str}),
            ('league_hustle_team_leaders', nba.nba_leaguehustlestatsteamleaders,
             {'season': season_str}),

            # Leaders
            ('league_leaders', nba.nba_leagueleaders,
             {'season': season_str}),

            # Standings
            ('league_standings', nba.nba_leaguestandings,
             {'season': season_str}),
            ('league_standings', nba.nba_leaguestandingsv3,
             {'season': season_str}),

            # Game logs
            ('league_game_log', nba.nba_leaguegamelog,
             {'season': season_str}),
        ]
    ...
    def scrape_static_endpoints(self):
        ...
        static_endpoints = [
            ...
            # Common info
            ('common_playoff_series', nba.nba_commonplayoffseries, {'season': '2023-24'}),
        ]
    ...
    def scrape_all(self, seasons, include_game_detail=False):
        ...
        if include_game_detail and schedule_data:
            ...
            games = schedule_data[:100] if len(schedule_data) > 100 else schedule_data  # Limit for testing
...
def main():
    ...
    parser.add_argument('--seasons', nargs='+', type=int,
                       help='Season years (e.g., 2023 2024 2025)')
    parser.add_argument('--all-seasons', action='store_true',
                       help='Scrape all seasons 2002-2025')
    parser.add_argument('--today', action='store_true',
                       help='Scrape just today\'s season (2025)')
    parser.add_argument('--game-detail', action='store_true',
                       help='Include per-game endpoints (SLOW!)')
    parser.add_argument('--output-dir', default='/tmp/hoopr_complete',
    ...
    scraper.scrape_all(
        seasons=seasons,
        include_game_detail=args.game_detail
    )
