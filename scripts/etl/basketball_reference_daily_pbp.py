...
Differences from daily box score scraper:
- Extracts ONLY PBP (no team/player stats)
- Stores in separate S3 path (pbp/ not box_scores/)
- Skips games without PBP table (common for historical games)
...
class DailyPBPScraper:
    ...
    def get_games_from_espn(self, date_str: str) -> List[Dict]:
    ...
    def run(self, dates: List[str]):
        ...
        print("\n" + "="*70)
        print("BASKETBALL REFERENCE DAILY PBP SCRAPER")
        print("="*70)
        ...
        for date in dates:
            ...
            games = self.get_games_from_espn(date)
        ...
        estimated_minutes = (total_games * RATE_LIMIT) / 60
        print(f"Estimated time: {estimated_minutes:.1f} minutes ({total_games} games × {RATE_LIMIT}s)\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Basketball Reference daily PBP scraper"
    )
    parser.add_argument(
        "--date",
        type=str,
    ...
    scraper = DailyPBPScraper(
        dry_run=args.dry_run,
        upload_to_s3=not args.no_s3
    )

    try:
        scraper.run(dates=dates)
    finally:
        scraper.cleanup()
