...
class NBAAPIPossessionDetector:
    ...
    def is_possession_end(event, next_event=None):
        ...
        if event_type == 3:  # FREE_THROW
            ...
            desc = (event.get('HOMEDESCRIPTION') or
...
def main():
    parser = argparse.ArgumentParser(description='Build possession panel from NBA API data')
    ...
    logger.info(f"  Average: {len(all_possessions) / len(pbp_files):.1f} possessions per game")
    ...
    if all_possessions:
        ...
        cur.copy_from(
            ...
            sep=',',
            columns=['game_id', 'possession_number', 'period', 'time_remaining',
                    'offense_team_id', 'start_score_margin', 'points_scored',
                    'possession_result', 'num_events']
        )
        conn.commit()

        logger.info(f"✅ Wrote {len(df):,} possessions to possession_panel_nba_api table")
