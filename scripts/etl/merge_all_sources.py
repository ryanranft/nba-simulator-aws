...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/master_etl.log'),
...
class MasterDatabaseETL:
    ...
    def __init__(self, s3_bucket: str = 'nba-sim-raw-data-lake', 
        ...
        self.data_sources = {
            ...
            'nba_stats': DataSourceConfig('nba_stats', 'nba_api_comprehensive/', priority=1),
            'hoopr': DataSourceConfig('hoopr', 'hoopr_parquet/', priority=3),
            'basketball_reference': DataSourceConfig('basketball_reference', 'basketball_reference/', priority=4),
            'kaggle': DataSourceConfig('kaggle', None, local_path='data/kaggle/nba.sqlite', priority=5)
        }
        ...
        logger.info(f"MasterDatabaseETL initialized. S3 Bucket: {self.s3_bucket}, Cache: {self.local_cache_dir}")
    ...
    def extract_nba_stats_data(self, season: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Extracted NBA Stats data: {sum(len(v) for v in nba_stats_data.values())} files")
    ...
    def extract_basketball_reference_data(self, season: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Extracted Basketball Reference data: {sum(len(v) for v in bref_data.values())} files")
                    ...
                    for _, row in players_df.iterrows():
                        ...
                        if player_id not in players_by_id:
                            players_by_id[player_id] = {
                                ...
                                'source_player_ids': {source_name: str(row.get('id', ''))},
                                'data_sources': [source_name]
                            }
                    ...
                    for _, row in teams_df.iterrows():
                        ...
                        if team_id not in teams_by_id:
                            teams_by_id[team_id] = {
                                ...
                                'source_team_ids': {source_name: str(row.get('id', ''))},
                                'data_sources': [source_name]
                            }
    ...
    def _load_players_to_db(self, players: List[Dict[str, Any]]) -> None:
        ...
        try:
            with conn.cursor() as cur:
                ...
                columns = ['player_id', 'player_name', 'first_name', 'last_name', 
                          'position', 'height_inches', 'weight_lbs', 'source_player_ids', 'data_sources']
    ...
    def _load_teams_to_db(self, teams: List[Dict[str, Any]]) -> None:
        ...
        try:
            with conn.cursor() as cur:
                columns = ['team_id', 'team_name', 'team_abbreviation', 'conference', 
                          'division', 'source_team_ids', 'data_sources']
        ...
        for source_name, config in self.data_sources.items():
            ...
            try:
                if source_name == 'espn':
                    ...
                    all_data[source_name] = self.extract_basketball_reference_data(season)
                elif source_name == 'kaggle':
...
def main():
    ...
    parser.add_argument('--cache-dir', default='/tmp/nba_data_cache', help='Local cache directory')
