...
def get_rds_summary(conn_rds):
    ...
    if "temporal_events" in tables:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM temporal_events WHERE data_source = 'espn'"
        )
...
def generate_report(local_summary, rds_summary, missing_games, quality, output_path):
    ...
    lines.append("")
    lines.append(
        f"- **Local Database:** {local_summary['total_games']:,} games, {local_summary['total_events']:,} events"
    )
    lines.append(
        f"- **RDS Database:** {rds_summary['total_games']:,} games, {rds_summary.get('espn_events', 0):,} ESPN events"
    )
    lines.append(
        f"- **Missing in RDS:** {len(missing_games['missing_in_rds']):,} games"
    )
    ...
    lines.append(
        f"**Games with Play-by-Play:** {local_summary['games_with_pbp']:,} ({local_summary['games_with_pbp']/local_summary['total_games']*100:.1f}%)"
    )
    lines.append(f"**Total Play-by-Play Events:** {local_summary['total_events']:,}")
    lines.append(
        f"**Date Range:** {local_summary['date_range'][0]} to {local_summary['date_range'][1]}"
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"**Tables Present:** {', '.join(rds_summary['tables_exist']) if rds_summary['tables_exist'] else 'None'}"
    )
    lines.append(f"**Total Games:** {rds_summary['total_games']:,}")
    lines.append(f"**ESPN Events:** {rds_summary.get('espn_events', 0):,}")
    lines.append(
        f"**Total Events (All Sources):** {rds_summary.get('total_events', 0):,}"
    )

    if rds_summary["date_range"][0]:
        lines.append(
            f"**Date Range:** {rds_summary['date_range'][0]} to {rds_summary['date_range'][1]}"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"**Games in Local but NOT in RDS:** {len(missing_games['missing_in_rds']):,}"
    )
    lines.append(
        f"**Games in RDS but NOT in Local:** {len(missing_games['extra_in_rds']):,}"
    )
    lines.append("")
    ...
    lines.append(
        f"- Missing home team: {quality['missing_fields']['missing_home_team']:,}"
    )
    lines.append(
        f"- Missing away team: {quality['missing_fields']['missing_away_team']:,}"
    )
    ...
    lines.append("")
    lines.append(
        "| Year | Total Games | Games with PBP | Coverage % | Avg Events/Game |"
    )
    lines.append(
        "|------|-------------|----------------|------------|-----------------|"
    )
    ...
    for season, total, with_pbp, coverage, avg_events in quality["pbp_coverage"]:
        lines.append(
            f"| {season} | {total:,} | {with_pbp:,} | {coverage:.1f}% | {avg_events:.0f} |"
        )
    ...
    if len(missing_games["missing_in_rds"]) > 0:
        lines.append(
            f"1. **Load missing games to RDS:** {len(missing_games['missing_in_rds']):,} games need to be loaded"
        )
    ...
    if rds_summary["total_games"] == 0:
        ...
        lines.append(
            f"   - Extract schedule: `python scripts/etl/extract_schedule_local.py`"
        )
    ...
    if quality["missing_fields"]["zero_scores"] > 100:
        lines.append(
            "3. **Many games with zero scores:** Likely future or cancelled games"
        )
        lines.append(
            f"   - {quality['missing_fields']['zero_scores']:,} games affected"
        )
    ...
    lines.append("")
    lines.append(
        f"*Report generated by: `scripts/analysis/compare_espn_local_vs_rds.py`*"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", help="Output report path (default: print to stdout)"
    )
    parser.add_argument(
        "--no-rds",
        action="store_true",
        help="Skip RDS comparison (local analysis only)",
    )
    ...
    if not args.no_rds:
        ...
        try:
            ...
            print(
                f"✓ Found {len(missing_games['missing_in_rds']):,} games missing in RDS"
            )
