...
def analyze_directory(dir_path: Path, check_func, file_type: str, sample_size: int = 100) -> Dict:
    """
    ...
    print(f"  ✅ Valid:  {len(valid_files):3d} / {len(sample_files)} ({valid_pct:5.1f}%)")
    print(f"  ❌ Empty:  {len(empty_files):3d} / {len(sample_files)} ({empty_pct:5.1f}%)")
    print(f"  ⚠️  Errors: {len(error_files):3d} / {len(sample_files)} ({error_pct:5.1f}%)")
    print()
    ...
    if valid_files:
        ...
        print(f"  Range:   {min(valid_sizes)/1024:.1f} KB - {max(valid_sizes)/1024:.1f} KB")
    ...
    if empty_files:
        ...
        print(f"  Range:   {min(empty_sizes)/1024:.1f} KB - {max(empty_sizes)/1024:.1f} KB")

    # Show sample valid and empty files
    print(f"\n🔍 Sample Valid Files:")
    for fname, result in valid_files[:3]:
        if file_type == 'play-by-play':
            print(f"  {fname} - {result['play_count']} plays, {result['file_size']/1024:.1f} KB")
        elif file_type == 'box_scores':
            print(f"  {fname} - {result['player_count']} players, {result['file_size']/1024:.1f} KB")
        elif file_type == 'schedule':
            print(f"  {fname} - {result['game_count']} games, {result['file_size']/1024:.1f} KB")
        elif file_type == 'team_stats':
            print(f"  {fname} - {result['team_count']} teams, {result['file_size']/1024:.1f} KB")
...
def main():
    ...
    results['pbp'] = analyze_directory(pbp_dir, check_pbp_file, 'play-by-play', sample_size=200)

    # Box scores
    box_dir = data_dir / 'nba_box_score'
    results['box'] = analyze_directory(box_dir, check_box_score_file, 'box_scores', sample_size=200)

    # Schedule
    schedule_dir = data_dir / 'nba_schedule_json'
    results['schedule'] = analyze_directory(schedule_dir, check_schedule_file, 'schedule', sample_size=200)

    # Team stats
    team_dir = data_dir / 'nba_team_stats'
    results['team'] = analyze_directory(team_dir, check_team_stats_file, 'team_stats', sample_size=200)
    ...
    print(f"\n📊 Overall Statistics:")
    print(f"  Total files:        {total_files:,}")
    print(f"  Est. valid files:   {total_valid:,} ({(total_valid/total_files*100):.1f}%)")
    print(f"  Est. empty files:   {total_files - total_valid:,} ({((total_files-total_valid)/total_files*100):.1f}%)")

    print(f"\n💾 Storage Impact:")
    print(f"  Current S3:         119 GB (all files)")
    print(f"  Est. valid data:    ~{(total_valid/total_files*119):.1f} GB")
    print(f"  Est. waste:         ~{((total_files-total_valid)/total_files*119):.1f} GB")
