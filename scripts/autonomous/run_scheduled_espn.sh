#!/bin/bash
#
# ESPN Scheduled Scraper Runner with Post-Execution Hooks
#
# This script runs the ESPN incremental scraper and triggers post-execution hooks:
# - DIMS metrics update
# - Reconciliation cycle
#
# Designed to be called by cron at 2 AM daily
#
# Usage:
#   bash scripts/autonomous/run_scheduled_espn.sh [--days N] [--dry-run]
#
...
LOG_FILE="$LOG_DIR/espn_scheduled_$(date +%Y%m%d_%H%M%S).log"
...
# Error handler
error_handler() {
    log "ERROR: ESPN scheduled scraper failed at line $1"
    exit 1
}
...
log "========================================================================"
log "ESPN SCHEDULED SCRAPER - Starting"
log "========================================================================"
log "Days: $DAYS"
...
# Phase 1: Run ESPN scraper
log "Phase 1/3: Running ESPN incremental scraper (async)..."
if [ -n "$DRY_RUN" ]; then
    python scripts/etl/espn_incremental_async.py $DAYS --dry-run >> "$LOG_FILE" 2>&1
else
    python scripts/etl/espn_incremental_async.py $DAYS >> "$LOG_FILE" 2>&1
fi
SCRAPER_EXIT_CODE=$?

if [ $SCRAPER_EXIT_CODE -eq 0 ]; then
    log "✓ ESPN scraper completed successfully"
else
    log "✗ ESPN scraper failed with exit code $SCRAPER_EXIT_CODE"
...
log "Phase 2/3: Updating DIMS ESPN metrics..."
log "Note: DIMS update runs in background - may take 2-3 minutes for all metrics"
log "      ESPN metrics will be updated, but some non-ESPN metrics may timeout"

# Run DIMS update in background and continue (don't block)
python scripts/monitoring/dims_cli.py update --category espn_data >> "$LOG_FILE" 2>&1 &
DIMS_PID=$!
...
if kill -0 $DIMS_PID 2>/dev/null; then
    ...
    if [ $DIMS_EXIT_CODE -eq 0 ]; then
        ...
        log "⚠ DIMS update completed with some failures (exit code: $DIMS_EXIT_CODE)"
        log "  Some metrics failed to update (non-critical - continuing)"
    fi
    # Force success - DIMS metric failures should not fail the entire scraper
    DIMS_EXIT_CODE=0
fi
...
log ""
log "========================================================================"
log "ESPN SCHEDULED SCRAPER - Complete"
log "========================================================================"
