...
def reconstruct_wall_clock(
    game_start: datetime, period: int, clock_seconds: int
) -> datetime:
    """
    ...
    try:
        ...
        if period <= 4:
            # Regular game
            quarter_breaks = (
                max(0, completed_quarters - 1) * 2 * 60
...
def process_batch(df: pd.DataFrame, team_map: dict) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        try:
            ...
            period = row["period"]
            wall_clock_utc = reconstruct_wall_clock(
                game_start, period, game_clock_seconds
            )
            ...
            processed_rows.append(
                {
                    ...
                    "game_clock_seconds": (
                        ...
                        if game_clock_seconds is not None
                        else None
                    ),
...
def load_events(test_mode=False):
    ...
    try:
        ...
        for chunk in pd.read_csv(
            ...
            processed_df["game_clock_seconds"] = processed_df[
                "game_clock_seconds"
            ...
            if batch_num % 10 == 0:
                ...
                print(
                    f"  Batch {batch_num}/{total_batches}: {loaded_count:,}/{total_lines:,} events ({pct:.1f}%)"
                )
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Test mode: load first 100k events only"
    )
