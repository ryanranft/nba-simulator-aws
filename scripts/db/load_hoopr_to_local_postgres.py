...
def load_csv_to_table(conn, csv_path, table_name, season=None):
    ...
    try:
        ...
        if table_name == 'hoopr_play_by_play':
            ...
            keep_cols = ['game_id', 'season', 'game_date', 'event_num', 'period', 'clock',
                        'team_id', 'player_id', 'event_type', 'description', 'score_home', 'score_away']
            ...
            keep_cols = ['game_id', 'season', 'season_type', 'game_date', 'team_id', 'player_id',
                        'player_name', 'minutes', 'fgm', 'fga', 'fg_pct', 'fg3m', 'fg3a', 'fg3_pct',
                        'ftm', 'fta', 'ft_pct', 'oreb', 'dreb', 'reb', 'ast', 'stl', 'blk',
                        'tov', 'pf', 'pts', 'plus_minus', 'starter']
            df = df[[col for col in keep_cols if col in df.columns]]
            ...
            # Select only columns that exist in schema
            keep_cols = ['game_id', 'season', 'game_date', 'team_id', 'team_name', 'fgm', 'fga',
            ...
            # Select only columns that exist in schema
            keep_cols = ['game_id', 'season', 'game_date', 'home_team_id', 'away_team_id',
                        'home_team_name', 'away_team_name', 'home_score', 'away_score', 'game_status']
            df = df[[col for col in keep_cols if col in df.columns]]
...
def main():
    ...
    log("=" * 80)
    player_box_files = sorted(glob.glob(os.path.join(DATA_DIR, 'bulk_player_box', '*.csv')))
    for pbox_file in player_box_files:
        season = int(os.path.basename(pbox_file).replace('player_box_', '').replace('.csv', ''))
    ...
    for tbox_file in team_box_files:
        season = int(os.path.basename(tbox_file).replace('team_box_', '').replace('.csv', ''))
    ...
    for sched_file in schedule_files:
        season = int(os.path.basename(sched_file).replace('schedule_', '').replace('.csv', ''))
    ...
    log("=" * 80)
    player_stats_files = sorted(glob.glob(os.path.join(DATA_DIR, 'league_dashboards', 'player_stats_*.csv')))
    for pstats_file in player_stats_files:
        season = int(os.path.basename(pstats_file).replace('player_stats_', '').replace('.csv', ''))
    ...
    log("=" * 80)
    team_stats_files = sorted(glob.glob(os.path.join(DATA_DIR, 'league_dashboards', 'team_stats_*.csv')))
    for tstats_file in team_stats_files:
        season = int(os.path.basename(tstats_file).replace('team_stats_', '').replace('.csv', ''))
    ...
    log("Loading lineups...")
    log("=" * 80)
    lineup_files = sorted(glob.glob(os.path.join(DATA_DIR, 'league_dashboards', 'lineups_5man_*.csv')))
    for lineup_file in lineup_files:
        season = int(os.path.basename(lineup_file).replace('lineups_5man_', '').replace('.csv', ''))
    ...
    log("=" * 80)
    standings_files = sorted(glob.glob(os.path.join(DATA_DIR, 'standings', 'standings_*.csv')))
    for standings_file in standings_files:
        season = int(os.path.basename(standings_file).replace('standings_', '').replace('.csv', ''))
