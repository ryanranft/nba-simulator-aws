...
SEVERITY_THRESHOLDS = {
    ...
    "date": {"LOW": 0, "MEDIUM": 1, "HIGH": 1},  # Same date  # 1 day off  # >1 day off
}
...
def calculate_severity(
    field_name: str, difference: float, pct_difference: float
) -> str:
...
def detect_discrepancies(
    game_id: str, espn_data: Dict, hoopr_data: Dict, unified_conn, verbose: bool = False
) -> List[Dict]:
    ...
    if espn_data["event_count"] is not None and hoopr_data["event_count"] is not None:
        ...
        if espn_count != hoopr_count:
            ...
            if verbose:
                print(
                    f"  ⚠️  event_count: ESPN={espn_count}, hoopR={hoopr_count}, diff={difference} ({pct_diff:.1f}%), severity={severity}"
                )
    ...
    if espn_data["home_score"] is not None and hoopr_data["home_score"] is not None:
        ...
        if espn_score != hoopr_score:
            ...
            pct_diff = (
                (difference / max(espn_score, hoopr_score) * 100)
                if max(espn_score, hoopr_score) > 0
                else 0
            )
            ...
            if verbose:
                print(
                    f"  ⚠️  home_score: ESPN={espn_score}, hoopR={hoopr_score}, diff={difference}, severity={severity}"
                )
    ...
    if espn_data["away_score"] is not None and hoopr_data["away_score"] is not None:
        ...
        if espn_score != hoopr_score:
            ...
            if verbose:
                print(
                    f"  ⚠️  away_score: ESPN={espn_score}, hoopR={hoopr_score}, diff={difference}, severity={severity}"
                )
    ...
    if espn_data["game_date"] != hoopr_data["game_date"]:
        ...
        if verbose:
            print(
                f"  ⚠️  game_date: ESPN={espn_data['game_date']}, hoopR={hoopr_data['game_date']}, severity=HIGH"
            )
...
def update_quality_scores(unified_conn, game_id: str, discrepancies: List[Dict]):
    ...
    has_score_issue = any(
        d["field_name"] in ["home_score", "away_score"] for d in discrepancies
    )
...
def process_games(
    ...
    for i, game in enumerate(games, 1):
        ...
        discrepancies = detect_discrepancies(
            espn_game_id, espn_data, hoopr_data, unified_conn, verbose=verbose
        )
        ...
        if discrepancies:
            ...
            if verbose or (i <= 10):
                print(
                    f"[{i}/{total_games}] ⚠️  {espn_game_id}: {len(discrepancies)} discrepancies"
                )
        else:
            if verbose and i <= 10:
        ...
        if i % 1000 == 0:
            ...
            print(
                f"  Progress: {i:,}/{total_games:,} ({pct:.1f}%) | "
    ...
    print(
        f"Games with discrepancies:    {games_with_discrepancies:,} ({games_with_discrepancies/total_games*100:.1f}%)"
    )
    print(
        f"Games with perfect agreement: {total_games - games_with_discrepancies:,} ({(total_games - games_with_discrepancies)/total_games*100:.1f}%)"
    )
    ...
    if severity_counts:
        ...
        for severity in ["HIGH", "MEDIUM", "LOW"]:
            if severity in severity_counts:
                ...
                print(
                    f"  {severity:10s} {count:,} ({count/total_discrepancies*100:.1f}%)"
                )
        print()
...
Result:
  ...
  - Source coverage marked with discrepancy flags
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of games to analyze (for testing)"
    )
    ...
    parser.add_argument(
        "--game-id", type=str, help="Analyze single game by ESPN game ID"
    )
    ...
    # Process games
    process_games(
        espn_conn, hoopr_conn, unified_conn, espn_to_hoopr, games, verbose=args.verbose
    )
