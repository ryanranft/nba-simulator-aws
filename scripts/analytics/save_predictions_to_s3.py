...
def generate_sample_predictions(n_games=100):
    ...
    results = pd.DataFrame({
        'game_id': [f'GAME_{today.strftime("%Y%m%d")}_{i:04d}' for i in range(n_games)],
...
def save_predictions_to_s3(predictions_df, season='2024-25'):
    ...
    predictions_df['partition_date'] = predictions_df['game_date'].dt.strftime('%Y-%m-%d')

    dates = predictions_df['partition_date'].unique()

    for date in dates:
        date_predictions = predictions_df[predictions_df['partition_date'] == date].copy()
    ...
    response = s3.list_objects_v2(
        Bucket=S3_BUCKET,
        Prefix=f'{S3_PREDICTIONS_PREFIX}/season={season}/'
    )
...
def main():
    ...
    print(f"  Date range: {predictions['game_date'].min()} to {predictions['game_date'].max()}")
