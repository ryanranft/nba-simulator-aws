...
def process_all_espn_data(limit: Optional[int] = None):
    ...
    for i, pbp_file in enumerate(pbp_files):
        if (i + 1) % 5000 == 0:
            print(
                f"Progress: {i+1:,}/{len(pbp_files):,} games ({(i+1)/len(pbp_files)*100:.1f}%)..."
            )
    ...
    print(
        f"Games with plays: {games_with_plays:,} ({games_with_plays/processed_count*100:.1f}%)"
    )
    ...
    print("Next steps:")
    print(
        "1. Load events to local PostgreSQL: python scripts/db/load_espn_to_local_postgres.py"
    )
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract local ESPN data to temporal format (UPDATED)"
    )
    parser.add_argument(
