...
class CrossSourceComparator:
    ...
    def compare_summary(self) -> Dict:
        ...
        complementary = self._identify_complementary_data(espn_stats, hoopr_stats, overlap)
    ...
    def _print_source_summaries(self, espn_stats: Dict, hoopr_stats: Dict):
        ...
        print(f"  Date range:           {espn_stats['date_range'][0]} to {espn_stats['date_range'][1]}")
        ...
        print(f"  Date range:           {hoopr_stats['date_range'][0]} to {hoopr_stats['date_range'][1]}")
    ...
    def _compare_overlap_coverage(self, espn_stats: Dict, hoopr_stats: Dict, overlap: Dict) -> Dict:
        ...
        return {
            ...
            'event_diff_pct': ((hoopr_events - espn_events) / espn_events * 100) if espn_events > 0 else 0
        }

    def _identify_complementary_data(self, espn_stats: Dict, hoopr_stats: Dict, overlap: Dict) -> Dict:
        ...
        return {
            'espn_unique_games': espn_stats['total_games'] - espn_stats['overlap_games'],
            'hoopr_fills_espn_gaps': hoopr_stats['overlap_games'] - espn_stats['overlap_games'],
            'espn_quality_score': 90,
            'hoopr_quality_score': 80,
            'recommended_strategy': 'Use ESPN (1993-2001) + hoopR (2002-2025)'
        }

    def _print_assessment(self, espn_stats: Dict, hoopr_stats: Dict,
                         overlap_stats: Dict, complementary: Dict):
        ...
        if 2 <= event_diff_pct <= 5:
            ...
            issues.append(f"Event count difference high (+{event_diff_pct:.1f}%, expected +2-5%)")
        elif event_diff_pct < -5:
            issues.append(f"ESPN has significantly more events (unexpected)")

        # Check game coverage
        if overlap_stats['hoopr_games'] > overlap_stats['espn_games']:
            successes.append(f"hoopR fills {overlap_stats['game_diff']:,} ESPN PBP gaps")
        else:
        ...
        print("3. ⏸️  Cross-validate in RDS: python scripts/utils/compare_espn_hoopr_rds.py")
        print("4. ⏸️  Verify local matches cloud: python scripts/utils/verify_local_cloud_sync.py")
    ...
    def detailed_comparison(self):
        ...
        for game_id in list(in_both)[:1000]:  # Sample first 1000
            ...
            diff_pct = ((hoopr_count - espn_count) / espn_count * 100) if espn_count > 0 else 0
    ...
    def export_report(self, output_path: str = None):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("source,total_games,games_with_pbp,total_events,date_start,date_end\n")
            f.write(f"ESPN,{results['espn_stats']['total_games']},{results['espn_stats']['games_with_pbp']},"
                   f"{results['espn_stats']['total_events']},{results['espn_stats']['date_range'][0]},"
                   f"{results['espn_stats']['date_range'][1]}\n")
            f.write(f"hoopR,{results['hoopr_stats']['total_games']},{results['hoopr_stats']['games_with_pbp']},"
            ...
            f.write(f"overlap_period,{results['overlap']['start_date']} to {results['overlap']['end_date']}\n")
            ...
            f.write(f"hoopr_overlap_events,{results['overlap_stats']['hoopr_events']}\n")
            f.write(f"event_difference,{results['overlap_stats']['event_diff']}\n")
            f.write(f"event_difference_pct,{results['overlap_stats']['event_diff_pct']:.2f}%\n")
