...
class FileInventory:
        ...
        try:
            ...
            if filepath.suffix == ".py":
                ...
                func_matches = re.findall(
                    r"^def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(", content, re.MULTILINE
                )
                functions.extend(
                    [f"{name}()" for name in func_matches[:5]]
                )  # Limit to 5
        ...
        try:
            ...
            if filepath.suffix == ".py":
                ...
                from_matches = re.findall(
                    r"^from\s+([a-zA-Z_][a-zA-Z0-9_.]*)", content, re.MULTILINE
                )
                ...
                external = [
                    ...
                    if imp not in ["os", "sys", "re", "datetime", "pathlib", "typing"]
                ]
    ...
    def generate_markdown(self) -> str:
        ...
        for category, files in categories.items():
            ...
            for filepath in sorted(files):
                ...
                if info["dependencies"]:
                    output.append(
                        f"**Dependencies:** {', '.join(info['dependencies'])}"
                    )
        ...
        output.append("")
        total_lines = sum(
            self.get_file_info(f)["lines"] for f in self.files_to_document
        )
