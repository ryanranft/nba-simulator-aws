...
class PredictionValidator:
    ...
    def validate_predictions(
        ...
        score_metrics = {}
        if (
            "predicted_home_score" in df_merged.columns
            and "predicted_away_score" in df_merged.columns
        ):
            # Filter out rows with missing predicted scores
            df_with_scores = df_merged[
                df_merged["predicted_home_score"].notna()
                & df_merged["predicted_away_score"].notna()
            ].copy()

            if len(df_with_scores) > 0:
                # Calculate score differences
                df_with_scores["home_score_error"] = (
                    df_with_scores["predicted_home_score"]
                    - df_with_scores["home_score"]
                )
                df_with_scores["away_score_error"] = (
                    df_with_scores["predicted_away_score"]
                    - df_with_scores["away_score"]
                )
                ...
                home_score_rmse = np.sqrt(
                    (df_with_scores["home_score_error"] ** 2).mean()
                )
                away_score_rmse = np.sqrt(
                    (df_with_scores["away_score_error"] ** 2).mean()
                )
                ...
                if "home_win" in df_with_scores.columns:
                    ...
                    actual_home_win = (
                        (df_with_scores["actual_winner"] == df_with_scores["home_team"])
                        .astype(int)
                        .values
                    )
    ...
    def generate_report(self, validation_results: Dict, output_path: str = None):
        ...
        # Score accuracy metrics
        if (
            "score_metrics" in validation_results
            and validation_results["score_metrics"]
        ):
            ...
            print(
                f"  Total Points MAE: {metrics.get('total_points_mae', 0):.2f} points"
            )
            print(f"  Margin MAE: {metrics.get('margin_mae', 0):.2f} points")
            print(
                f"  Brier Score: {metrics.get('brier_score', 0):.4f} (lower is better)"
            )
        ...
        if "details" in validation_results:
            ...
            for _, game in df_details.iterrows():
                ...
                # Winner prediction
                print(
                    f"  Winner: Predicted {game['predicted_winner']}, Actual {game['actual_winner']}"
                )

                # Score prediction
                if pd.notna(game.get("predicted_home_score")) and pd.notna(
                    game.get("predicted_away_score")
                ):
                    print(
                        f"  Score: Predicted {game['home_team']} {game['predicted_home_score']:.1f} - {game['away_team']} {game['predicted_away_score']:.1f}"
                    )
                    print(
                        f"         Actual    {game['home_team']} {game['home_score']} - {game['away_team']} {game['away_score']}"
                    )
                    ...
                    print(
                        f"  Errors: Home {home_error:+.1f}, Away {away_error:+.1f}, Total {total_error:+.1f}"
                    )
                else:
                    print(
                        f"  Score: Actual {game['home_team']} {game['home_score']} - {game['away_team']} {game['away_score']}"
                    )

                print(
                    f"  Strength: {game['prediction_strength']}, Confidence: {game['confidence']:.1%}"
                )
