#!/usr/bin/env python3
"""
NBA Game Prediction - Panel Data Enhanced Model Training

Integrates:
- rec_22: Panel Data Processing System
- rec_11: Advanced Feature Engineering Pipeline
- ml_systems_1: MLflow for model versioning
...
# Add paths for book recommendation implementations
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../../docs/phases/phase_0"))
sys.path.insert(
    0,
    os.path.join(
        os.path.dirname(__file__),
        "../../docs/phases/phase_0/8.0001_statistical_frameworks",
    ),
)
sys.path.insert(
    0,
    os.path.join(
        os.path.dirname(__file__), "../../docs/phases/phase_0/8.0002_data_analysis"
    ),
)
...
# MLOps implementations (ml_systems_1 + ml_systems_2)
from implement_consolidated_ml_systems_1 import (
    ModelVersioningWithMlflow,
    MLFLOW_AVAILABLE,
)
from implement_consolidated_ml_systems_2 import DataDriftDetection, SCIPY_AVAILABLE
...
class PanelDataMLPipeline:
    ...
    def __init__(self, enable_mlflow=True, enable_drift_detection=True):
        ...
        if self.enable_mlflow:
            self.mlflow_config = {
                ...
                "experiment_name": "nba-panel-data-predictions",  # New experiment
                "artifact_location": f"s3://{S3_BUCKET}/mlflow-panel",
                "default_tags": {
                    "project": "nba-simulator-aws",
                    "phase": "5-enhanced",
                    "data_type": "panel",
                    "features": "rec_22+rec_11",
                    "environment": "development",
                },
            }
            self.mlflow_manager = ModelVersioningWithMlflow(self.mlflow_config)
            print("  ✓ MLflow integration enabled (panel experiment)")
        ...
        if self.enable_drift_detection:
            self.drift_config = {
                ...
                "mlflow_tracking": self.enable_mlflow,
            }
            self.drift_detector = DataDriftDetection(self.drift_config)
            print("  ✓ Drift detection enabled (80+ features)")
    ...
    def create_panel_features(self):
        ...
        # Execute feature engineering (generates 80-100+ features)
        train_results = self.feature_system.execute(demo_data=panel_train.reset_index())
        ...
        print(f"    Features generated: {train_results['total_features']}")
        print(f"    New features added: {train_results['new_features']}")
        print(f"    Selected features: {train_results['selected_features']}")

        # Store results
        self.panel_train = train_results
        self.panel_test_shape = test_panel.shape
        ...
        self.train_df_panel = self._aggregate_panel_to_game(panel_train, train_df_orig)
        ...
        for idx, row in game_df.iterrows():
            for player_num in range(players_per_game):
                player_row = {
                    ...
                    "minutes": np.random.normal(28, 8).clip(15, 40),
                    # Shooting stats
    ...
    def prepare_ml_data(self):
        ...
        if self.enable_drift_detection:
            print("\n  Setting up drift detection with panel features...")
            self.drift_config["reference_data"] = self.X_train
            self.drift_detector = DataDriftDetection(self.drift_config)
            self.drift_detector.setup()
            print(f"  ✓ Drift detector configured ({len(self.feature_cols)} features)")
    ...
    def check_test_drift(self):
        ...
        if drift_results["summary"]["overall_drift_detected"]:
            ...
            for alert in drift_results["alerts"][:5]:
                print(f"      {alert}")
    ...
    def print_summary(self, comparison_df):
        ...
        print(f"  ✓ ml_systems_1: MLflow Tracking")
        print(f"  ✓ ml_systems_2: Drift Detection")
...
def main():
    ...
    print("  3. ml_systems_1: MLflow Model Versioning")
    print("  4. ml_systems_2: Data Drift Detection")
