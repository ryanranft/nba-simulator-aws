    ...
    if os.path.exists(cred_file):
        with open(cred_file) as f:
            for line in f:
                line = line.strip()
                if (
                    "DB_PASSWORD" in line or "NBA_SIM_DB_PASSWORD" in line
                ) and "=" in line:
...
def main():
    ...
    games = games.rename(
        columns={"home_score": "home_points", "away_score": "away_points"}
    )
    ...
    def calculate_rolling_stats(games_df, team_id, is_home=True, window=10):
        ...
        team_games["rolling_win_pct"] = (
            team_games["won"].rolling(window=window, min_periods=1).mean().shift(1)
        )
        team_games["rolling_ppg"] = (
            team_games["points_for"]
            .rolling(window=window, min_periods=1)
            .mean()
            .shift(1)
        )
        team_games["rolling_papg"] = (
        ...
        team_games["rolling_margin"] = (
            team_games["rolling_ppg"] - team_games["rolling_papg"]
        )
    ...
    for team_id in all_teams:
        ...
        home_rest = home_rest.rename(
            columns={"rest_days": "home_rest_days", "back_to_back": "home_back_to_back"}
        )
        home_rest_list.append(home_rest)

        away_rest = calculate_rest_days(games, team_id, is_home=False)
        away_rest = away_rest.rename(
            columns={"rest_days": "away_rest_days", "back_to_back": "away_back_to_back"}
        )
    ...
    games_with_stats["season_phase"] = (
        games_with_stats["month"]
        .map(
            {
                10: 0,
                11: 0,
                ...
                4: 2,
                5: 2,
                6: 2,  # Late season (includes playoffs)
            }
        )
        .fillna(0)
        .astype(int)
    ...
    print(
        f"  Dropped: {len(features_df) - len(features_df_clean):,} rows with missing values"
    )
    ...
    print(f"\nTrain/Test Split:")
    print(
        f"  Train: {len(train_df):,} rows ({train_df['game_date'].min()} to {train_df['game_date'].max()})"
    )
    print(
        f"  Test:  {len(test_df):,} rows ({test_df['game_date'].min()} to {test_df['game_date'].max()})"
    )
    ...
    try:
        response = s3.head_object(
            Bucket=S3_BUCKET, Key=f"{S3_PREFIX}/game_features.parquet"
        )
