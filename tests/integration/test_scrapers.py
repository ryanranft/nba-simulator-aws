...
class TestESPNScraper:
    ...
    async def test_scrape_schedule_with_mock_data(self, espn_config):
        ...
        async with ESPNScraper(espn_config) as scraper:
            with patch.object(scraper, 'fetch_url', new_callable=AsyncMock) as mock_fetch:
                with patch.object(scraper, 'parse_json_response', new_callable=AsyncMock) as mock_parse:
...
class TestConvenienceFunctions:
    ...
    async def test_scrape_nba_api_season_function(self):
        ...
        with patch('nba_simulator.etl.extractors.nba_api.scraper.NBAAPIScraper') as MockScraper:
            mock_instance = Mock()
            ...
            mock_instance.get_error_summary = AsyncMock(return_value={'total_errors': 0})
    ...
    async def test_scrape_hoopr_season_function(self):
        ...
        with patch('nba_simulator.etl.extractors.hoopr.scraper.HoopRScraper') as MockScraper:
            mock_instance = Mock()
            mock_instance.__aenter__ = AsyncMock(return_value=mock_instance)
            mock_instance.__aexit__ = AsyncMock(return_value=None)
