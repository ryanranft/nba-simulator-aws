...
class TestBasketballReferenceScheduleScraping:
    ...
    @pytest.mark.asyncio
    async def test_scrape_historical_season(
        self, bbref_scraper, mock_bbref_schedule_html
    ):
    ...
    @pytest.mark.asyncio
    async def test_scrape_schedule_month_filter(
        self, bbref_scraper, mock_bbref_schedule_html
    ):
            ...
            async with bbref_scraper:
                with pytest.raises(ValueError):
                    await bbref_scraper.scrape_player_season_stats(
                        player_id=invalid_player_id, season=2024
                    )
...
class TestBasketballReferencePlayByPlayScraping:
    ...
    @pytest.mark.asyncio
    async def test_scrape_pbp_all_quarters(self, bbref_scraper, mock_bbref_pbp_html):
...
class TestBasketballReferenceTeamScraping:
    ...
    @pytest.mark.asyncio
    async def test_scrape_team_season_stats(self, bbref_scraper, mock_bbref_team_html):
    ...
    @pytest.mark.asyncio
    async def test_scrape_team_roster(self, bbref_scraper, mock_bbref_team_html):
...
class TestBasketballReferenceErrorHandling:
    ...
    async def test_robots_txt_respect(self, bbref_scraper):
        ...
        assert (
            hasattr(bbref_scraper, "check_robots_txt")
            or bbref_scraper.config.rate_limit >= 3.0
        )
...
class TestBasketballReferenceDataValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_scraped_data(self, bbref_scraper, mock_bbref_boxscore_html):
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            async with bbref_scraper:
                box_score = await bbref_scraper.scrape_box_score(game_id, validate=True)

            # Should have validation metadata
            assert box_score.get("validated") is True

    @pytest.mark.asyncio
    async def test_data_quality_checks(self, bbref_scraper, mock_bbref_boxscore_html):
...
class TestBasketballReferenceS3Integration:
    ...
    @pytest.mark.asyncio
    async def test_save_to_s3(self, bbref_scraper, mock_bbref_boxscore_html):
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            with patch.object(bbref_scraper, "save_to_s3") as mock_save:
                async with bbref_scraper:
                    await bbref_scraper.scrape_box_score(game_id, save_s3=True)
    ...
    async def test_s3_key_generation(self, bbref_scraper):
        ...
        actual_key = bbref_scraper._generate_s3_key(
            data_type="box_score", game_id=game_id, date="2024-11-01"
        )
