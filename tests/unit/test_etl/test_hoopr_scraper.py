...
class TestHoopRScraperInitialization:
    ...
    def test_r_dependency_checking(self, hoopr_config):
        ...
        assert hasattr(scraper, "_check_r_installed") or hasattr(
...
class TestHoopRScheduleScraping:
    ...
    async def test_scrape_full_season_schedule(
        ...
        with patch("rpy2.robjects.r") as mock_r:
            ...
            async with hoopr_scraper_rpy2:
                games = await hoopr_scraper_rpy2.scrape_season_schedule(season=season)
...
class TestHoopRRosterScraping:
    ...
    @pytest.mark.asyncio
    async def test_scrape_team_roster(self, hoopr_scraper_rpy2, mock_hoopr_roster_data):
...
class TestHoopRBoxScoreScraping:
    ...
    @pytest.mark.asyncio
    async def test_scrape_box_score(self, hoopr_scraper_rpy2, mock_hoopr_boxscore_data):
...
class TestHoopRRIntegration:
    ...
    def test_r_script_generation(self, hoopr_scraper_subprocess):
        ...
        script = hoopr_scraper_subprocess._generate_r_script(
            function="nba_pbp", game_id=game_id
        )
...
class TestHoopRDependencyChecking:
    ...
    def test_fallback_to_espn_if_r_unavailable(self, hoopr_config):
        ...
        with patch.object(scraper, "_check_r_installed", return_value=False):
            ...
            assert hasattr(scraper, "fallback_to_espn") or hasattr(
                scraper, "_use_espn_fallback"
            )
...
class TestHoopRDataValidation:
    ...
    @pytest.mark.asyncio
    async def test_data_quality_checks(self, hoopr_scraper_rpy2, mock_hoopr_pbp_data):
...
class TestHoopRESPNCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_espn_data_format(self, hoopr_scraper_rpy2, mock_hoopr_pbp_data):
...
class TestHoopRS3Integration:
    ...
    @pytest.mark.asyncio
    async def test_save_to_s3(self, hoopr_scraper_rpy2, mock_hoopr_pbp_data):
        ...
        with patch("rpy2.robjects.r") as mock_r:
            ...
            with patch.object(hoopr_scraper_rpy2, "save_to_s3") as mock_save:
                async with hoopr_scraper_rpy2:
                    await hoopr_scraper_rpy2.scrape_play_by_play(game_id, save_s3=True)
    ...
    async def test_s3_key_generation(self, hoopr_scraper_rpy2):
        ...
        actual_key = hoopr_scraper_rpy2._generate_s3_key(
            data_type="play_by_play", game_id=game_id
        )
...
def mock_hoopr_roster_data():
    ...
    return [
        {"player_id": "2544", "name": "LeBron James", "position": "F", "number": "23"},
        {
            "player_id": "203076",
            "name": "Anthony Davis",
            "position": "F-C",
