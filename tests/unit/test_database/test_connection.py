...
class TestDatabaseQueries:
    ...
    def test_parameterized_query_execution(self, mock_db_connection):
        ...
        cursor.execute("SELECT COUNT(*) FROM games WHERE home_team = %s", ("LAL",))
        results = cursor.fetchall()
...
class TestDatabasePerformance:
    ...
    def test_query_uses_prepared_statements(self, mock_db_connection):
        ...
        for i in range(5):
            cursor.execute("SELECT * FROM games WHERE game_id = %s", (f"game_{i}",))
...
class TestDatabaseSecurity:
    ...
    def test_uses_parameterized_queries(self, mock_db_connection):
        ...
        cursor.execute("SELECT * FROM games WHERE home_team = %s", (team,))
