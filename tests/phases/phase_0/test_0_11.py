...
from validate_0_11 import Phase011Validator


class TestPhase011Validation:
    """Tests for Phase 0.11 validation."""

    @pytest.fixture
    def validator(self):
        """Create validator instance."""
        return Phase011Validator(verbose=False)
...
class TestPhase011Integration:
    """Integration tests for Phase 0.11."""

    def test_phase_complete_validation(self):
        """Comprehensive phase completion test."""
        validator = Phase011Validator(verbose=False)
        all_passed, results = validator.run_all_validations()

        assert all_passed == True, "Phase 0.11 validation failed"
