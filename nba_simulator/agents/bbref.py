...
class BasketballReferenceAgent(BaseAgent):
    ...
    def _execute_core(self) -> bool:
        ...
        try:
            ...
            if self.backfill_mode:
                ...
                tiers = [t for t in self.tiers_to_process 
            ...
            total_failed = sum(
                t['items_failed'] for t in self.tier_progress.values()
            )
    ...
    def _process_tier_generic(
        ...
        try:
            ...
            self.logger.info(
                f"Tier {tier_num}: Completed {expected_items} items"
            )
        ...
        for tier_num, tier_data in sorted(self.tier_progress.items()):
            if tier_data['status'] != 'pending':
                report['tiers_processed'].append({
                    ...
                    'success_rate': (
                        tier_data['items_successful'] / tier_data['items_processed'] * 100
                        if tier_data['items_processed'] > 0 else 0
                    )
                })
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'tiers_processed': len([
                t for t in self.tier_progress.values() 
