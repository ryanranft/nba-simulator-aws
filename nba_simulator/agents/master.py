...
class MasterAgent(BaseAgent):
    ...
    def register_agent(
        self, agent: BaseAgent, dependencies: Optional[Set[str]] = None
    ) -> None:
        """
    ...
    def _execute_phase(self, phase: ExecutionPhase, agent_names: List[str]) -> bool:
        """
    ...
    def _execute_agent_with_retry(self, agent_name: str) -> bool:
        ...
        self.log_error(
            f"Agent {agent_name} failed after {self.max_retries + 1} attempts"
        )
        return False
    ...
    def _check_dependencies(self, agent_name: str) -> bool:
        ...
        for dep_name in dependencies:
            if dep_name in self.failed_agents:
                self.log_error(f"Dependency {dep_name} failed for agent {agent_name}")
                return False
