...
class BaseWorkflow(ABC):
    ...
    def execute(self, async_mode: bool = False) -> bool:
        ...
        if self.state != WorkflowState.INITIALIZED and self.state != WorkflowState.READY:
    ...
    async def _execute_tasks_async(self) -> bool:
        ...
        for group in task_groups:
            ...
            if self.metrics.tasks_failed > 0 and not self.config.get('continue_on_error', False):
                return False
    ...
    async def _execute_task_async(self, task: WorkflowTask) -> bool:
        ...
        try:
            self.logger.info(f"Executing task (async): {task.task_id} ({task.task_name})")
            ...
            self.logger.info(f"Task {task.task_id} completed (async, duration={task.duration:.2f}s)")
            return True
    ...
    def _calculate_quality_score(self) -> None:
        ...
        # Penalize for errors
        error_penalty = (self.metrics.errors_encountered / self.metrics.total_tasks) * 15
    ...
    def cancel(self) -> bool:
        ...
        if self.state not in [WorkflowState.READY, WorkflowState.RUNNING, WorkflowState.PAUSED]:
    ...
    def _format_markdown_report(self, data: Dict[str, Any]) -> str:
        ...
        md += f"**Quality Score:** {data['execution_summary']['quality_score']:.1f}%\n\n"
