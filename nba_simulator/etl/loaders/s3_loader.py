...
class S3Loader(BaseLoader):
    ...
    async def _simple_upload(
        self, key: str, content: bytes, metadata: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _multipart_upload(
        ...
        response = self.s3_client.create_multipart_upload(
            Bucket=self.bucket, Key=key, **multipart_params
        )
...
class ESPNLoader(S3Loader):
    ...
    Uploads to ESPN-prefixed folders in s3://nba-sim-raw-data-lake/:
    - espn_box_scores/ - Game box score JSON files
    - espn_play_by_play/ - Play-by-play JSON files
    - espn_schedules/ - Schedule data
    - espn_team_stats/ - Team statistics
    """

    def __init__(self, data_type="box_scores", **kwargs):
        """
        Initialize ESPN S3 loader.

        Args:
            data_type: Type of ESPN data ('box_scores', 'play_by_play', 'schedules', 'team_stats')
            **kwargs: Additional arguments passed to S3Loader
        """
        # Map data types to ESPN-prefixed folders
        prefix_map = {
            "box_scores": "espn_box_scores/",
            "play_by_play": "espn_play_by_play/",
            "schedules": "espn_schedules/",
            "team_stats": "espn_team_stats/",
        }

        prefix = prefix_map.get(data_type, "espn_box_scores/")
...
class BasketballReferenceLoader(S3Loader):
    ...
    def __init__(self, tier: Optional[int] = None, **kwargs):
        ...
        super().__init__(
            prefix=prefix, content_type="application/json", compress=True, **kwargs
        )
