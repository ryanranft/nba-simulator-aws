...
class NBAAPIScraper(AsyncBaseScraper):
    ...
    def __init__(self, config: ScraperConfig, **kwargs):
        ...
        self.logger.info(f"Initialized NBA API scraper (rate_limit={config.rate_limit}s)")
    ...
    async def scrape_player_stats(
        ...
        data = await self.error_handler.retry_with_backoff(
            fetch_stats,
            metadata={'endpoint': 'player_stats', 'season': season}
        )
        ...
        # Store data
        if player_stats:
            filename = f"player_stats_{season.replace('-', '_')}_{measure_type.lower()}.json"
    ...
    async def scrape_team_stats(
        ...
        # Store data
        if team_stats:
            filename = f"team_stats_{season.replace('-', '_')}_{measure_type.lower()}.json"
            await self.store_data(
                team_stats,
                filename,
    ...
    async def scrape_box_score(
        ...
        if box_score:
            ...
            await self.store_data(
                box_score,
                filename,
                subdir=f"nba_api/box_scores/{game_id[:4]}"
            )
    ...
    async def scrape_game_details(self, game_id: str) -> Optional[Dict[str, Any]]:
        ...
        if any(game_data.values()):
            ...
            await self.store_data(
                game_data,
                filename,
                subdir=f"nba_api/games/{game_id[:4]}"
            )
    ...
    async def scrape_player_game_log(
        ...
        if game_log:
            ...
            await self.store_data(
                game_log,
                filename,
                subdir=f"nba_api/player_logs/{season.split('-')[0]}"
            )
    ...
    async def scrape_shot_chart(
        ...
        if shots:
            ...
            await self.store_data(
                shots,
                filename,
                subdir=f"nba_api/shot_charts/{season.split('-')[0]}"
            )
