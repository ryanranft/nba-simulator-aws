...
class HoopRScraper(AsyncBaseScraper):
    ...
    async def scrape_play_by_play(
        self, game_id: str, season: Optional[int] = None
    ...
    async def scrape_team_schedule(
        ...
        if valid_games:
            ...
            await self.store_data(
                valid_games, filename, subdir=f"hoopr/schedules/{season}"
            )
...
async def scrape_hoopr_season(
    ...
    results = {"teams_scraped": 0, "errors": 0}
...
if __name__ == "__main__":
    ...
    async def main():
        config = ScraperConfig(
            base_url="", rate_limit=1.0, output_dir="/tmp/hoopr_test"
        )
