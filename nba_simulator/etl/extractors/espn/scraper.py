...
class ESPNScraper(AsyncBaseScraper):
    ...
    async def scrape_schedule(
        ...
        data = await self.error_handler.retry_with_backoff(
            fetch_schedule,
            metadata={'url': url, 'season': season}
        )
        ...
        if valid_games:
            ...
            await self.store_data(
                valid_games,
                filename,
                subdir=f"espn/schedules/{season_str}"
            )
    ...
    async def scrape_game_details(self, game_id: str) -> Optional[Dict[str, Any]]:
        ...
        if any(game_data.values()):
            ...
            await self.store_data(
                game_data,
                filename,
                subdir=f"espn/games/{game_id[:4]}"  # Group by year
            )
    ...
    async def scrape_play_by_play(self, game_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        data = await self.error_handler.retry_with_backoff(
            fetch_pbp,
            metadata={'game_id': game_id, 'endpoint': 'playbyplay'}
        )
        ...
        if valid_plays:
            ...
            await self.store_data(
                valid_plays,
                filename,
                subdir=f"espn/play_by_play/{game_id[:4]}"
            )
