...
class BasketballReferenceScraper(AsyncBaseScraper):
    ...
    def __init__(self, config: ScraperConfig, **kwargs):
        ...
        if config.rate_limit < 3.0:
            logger.warning(f"Rate limit {config.rate_limit}s is too aggressive. Setting to 3.0s")
        ...
        self.logger.info(f"Initialized Basketball Reference scraper (rate_limit={config.rate_limit}s)")
    ...
    async def scrape_season_schedule(
        ...
        html = await self.error_handler.retry_with_backoff(
            fetch_schedule,
            metadata={'url': url, 'season': season, 'month': month}
        )
        ...
        if valid_games:
            ...
            await self.store_data(
                valid_games,
                filename,
                subdir=f"basketball_reference/schedules"
            )
    ...
    async def scrape_player_season_stats(
        ...
        if player_stats:
            ...
            await self.store_data(
                player_stats,
                filename,
                subdir=f"basketball_reference/players/{letter}"
            )
    ...
    def _parse_box_score_html(self, html: str, game_id: str) -> Optional[Dict[str, Any]]:
        """
...
async def scrape_basketball_reference_season(
    season: int,
    output_dir: str = "/tmp/bbref_scraper",
