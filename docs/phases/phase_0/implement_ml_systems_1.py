    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            artifact_location = self.config.get('artifact_location',
                                                's3://nba-sim-raw-data-lake/mlflow')
            ...
            if experiment is None:
                ...
                logger.info(f"  Created experiment: {self.experiment_name} (ID: {self.experiment_id})")
            else:
    ...
    def validate_prerequisites(self) -> bool:
        ...
        if artifact_location.startswith('s3://'):
            try:
                ...
                if credentials:
                    ...
                    logger.warning("⚠️  AWS credentials not found (S3 artifacts may fail)")
        ...
        if backend_uri.startswith('postgresql'):
            try:
                ...
                logger.error("❌ psycopg2 not installed (required for PostgreSQL backend)")
        ...
        if sys.version_info >= (3, 8):
            ...
            logger.warning(f"⚠️  Python {sys.version_info.major}.{sys.version_info.minor} (recommend 3.8+)")
        ...
        try:
            ...
            with mlflow.start_run(tags=self.default_tags) as run:
                ...
                try:
                    ...
                    logger.warning("  Scikit-learn not available, skipping model logging")
            ...
            logger.info(f"   Model: {model_name} v{registration.get('version')} (Production)")
            return results
    ...
    def cleanup(self):
        ...
        if self.experiment_id and MLFLOW_AVAILABLE:
            try:
                ...
                logger.info(f"  Experiment '{self.experiment_name}' has {len(runs)} runs")
            except Exception as e:
