...
try:
    ...
    logger.warning("scipy/numpy/pandas not installed. Run: pip install scipy numpy pandas")
...
class DataDriftDetection:
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            if reference_data_config is None:
                logger.warning("No reference data provided, will generate synthetic data for demo")
            ...
            if self.config.get('mlflow_tracking', False) and MLFLOW_AVAILABLE:
                mlflow.set_experiment(self.config.get('experiment_name', 'data-drift-monitoring'))
    ...
    def validate_prerequisites(self) -> bool:
        ...
        if sys.version_info >= (3, 7):
            ...
            logger.warning(f"⚠️  Python {sys.version_info.major}.{sys.version_info.minor} (recommend 3.7+)")
    ...
    def calculate_chi_squared(self, reference: pd.Series, current: pd.Series) -> Tuple[float, float]:
        """
    ...
    def calculate_jensen_shannon_divergence(self, reference: np.ndarray, current: np.ndarray, bins: int = 10) -> float:
        """
    ...
    def detect_drift(self, current_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"  Drift detected in {features_with_drift}/{total_features_checked} features ({results['summary']['drift_percentage']:.1f}%)")
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        try:
            ...
            current_data['field_goal_pct'] = current_data['field_goal_pct'] * 1.1  # Scale change
            # Change distribution for categorical
            current_data['home_away'] = np.random.choice(['home', 'away'], 500, p=[0.7, 0.3])
            ...
            if drift_results['summary']['overall_drift_detected']:
                logger.warning(f"   ⚠️  DRIFT DETECTED in {drift_results['summary']['features_with_drift']} features")
            else:
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self.drift_results:
                ...
                logger.info(f"    Total features monitored: {len(self.features_to_monitor)}")
                logger.info(f"    Reference data samples: {len(self.reference_data) if self.reference_data is not None else 0}")
            ...
            if self.feature_stats:
                logger.info(f"  Clearing feature statistics ({len(self.feature_stats)} features)")
